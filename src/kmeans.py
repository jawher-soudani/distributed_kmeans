...
class MRKMeans(MRJob):
    ...
    def combiner(self,k,data_points):
        #calculate a partial sum of points to prepare them for the average because we cannot calculate the average here
        first_sum_points = next(data_points)
        first_counter = 1
        for data_point in data_points:
            first_sum_points=[sum(x) for x in zip(first_sum_points,data_point)]
            first_counter += 1
        yield k, (first_sum_points, first_counter)
    
    def reducer(self, cluster, v):    
     # cluster is class and v are a first sum of points linked to the class with their counter
...
if __name__ == '__main__':
 ...
 #just run mapreduce !
    args = sys.argv[1:]
    mr_job=MRKMeans(args=[args[0],'--c',args[1],'--k',args[2]])
